...
- [ ] 3.1 TDD:`_cmd_proposal_review` — blocking(proposal) 時 `propose_attempts += 1`;超過 `--max-propose`(預設 3)改套用 `propose_retry_exceeded` → escalated
- [ ] 3.2 TDD:`_cmd_gate` — 失敗時 `gate_failures += 1`;超過 `--max-gate`(預設 3)改套用 `gate_retry_exceeded` → escalated(仍印失敗輸出、exit 1);gate_pass 不重置計數
