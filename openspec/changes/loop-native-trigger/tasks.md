...
- [ ] 1.1 TDD:從 `_cmd_arm_local` 抽出 `ensure_armed(checkpoint_path, heartbeat, …)`(注入 spawn/pid-alive 替身);`arm-local` 改薄殼,既有 arm-local 測試全綠
- [ ] 1.2 TDD:`config.py` 支援 `auto_arm` 讀取(缺檔/缺鍵預設 true)
- [ ] 1.3 TDD:12 個寫 checkpoint 的子命令(`start`/`event`/`gate`/`proposal-review`/`qa`/`leg-done`/`review`/`units-init`/`unit-done`/`unit-claim`/`unit-resolve`/`units-merge`)save 後自動 arm——參數化測試逐命令驗證;`resume_exec` 空靜默跳過;`auto_arm=false` 不 arm
