...
有些輸入的正確值只有在執行當下才知道(例如 pid),寫死在 fixture 裡沒有意義。
這種值一律以 `<大寫名稱>` 的佔位符寫進 `input` 與 `expect`,由**兩側消費者各自**
換成當下的實際值——替換規則必須兩側一致,否則等於兩側斷言不同的表。

目前用到的:

- `watcher.json` — `<SELF>` = 本行程的 pid(必然存活)、`<DEAD>` = 一個剛
  spawn 完並已 wait 收屍的 pid(必然不存在)。兩側都真的去 spawn `/usr/bin/true`
  再取它的 pid,而不是猜一個大數字:猜的數字有可能剛好被別的行程佔用,
  「reaped pid 應為 dead」那條就會變成偶發紅。
  佔位符也會出現在字串中間(例如 `"  <SELF>\n"`),所以替換要同時支援
  「整個值就是佔位符」與「字串內含佔位符」兩種情形。
