...
決策的真理來源在**引擎**:dispatch subagent 前跑

```
devloop model --stage <brainstorm|apply|review|fix> [--config .devloop/config.json]
```

stdout 印 alias(如 `sonnet`,dispatch 時帶為 model 參數)或 `inherit`(不帶 model 參數,subagent 用 session 模型)。**唯一例外**:架構性 fix 忽略引擎對 `fix` 的 budget 建議值、一律繼承(機械/架構之分是編排層判斷,引擎不碰)。`propose`、QA gate、proposal-review 恆繼承,不查引擎。

說明性附註(引擎內建的決議邏輯,勿在編排層重新實作):`models` override 優先 → `budget` 下 `apply`/`fix` = `sonnet`、其餘 None → `quality`(或未設)全 None。budget 的取捨:執行段(apply、機械 fix)換便宜模型省 output token,把關段留在 session 模型;對應護欄是 review code leg 改用 coverage-first 加重審查(正本 `references/review-coverage-first.md`)。

## 流程檔位與 UI/UX 線

兩條**正交軸**,brainstorm 時由編排自動判定、寫入 change meta、✋批准提案時明示供推翻;`devloop start --meta` 把兩軸**凍結進 checkpoint**(此後引擎 guard/hint 只讀 checkpoint;逃生門重 propose 不重判,要改就人工改 meta 後 `start --force` 重啟)。

- **`flow_profile`**(`full` 預設 | `light`):判定準則——docs/config/文案/微調 → light;功能或行為變更 → full;**不確定取 full**。light 的裁剪:設計縮為短節直接寫入 proposal(不產獨立草稿)、跳過✋批准設計(✋批准提案保留)、QA 以 `devloop event --event qa_skip` 誠實跳過(引擎 guard:light 且非 uiux 才放行,history 可審計)。**hard gate 與 review 恆不可裁**。`auto_approve=true` 時檔位建議自動視為接受。
- **`needs_uiux`**(UI 視覺與 UX 流程互動皆算):判定準則與各階段指示的唯一正本是 `references/uiux-thread.md`,勿即興。true 時:設計文件必含「UI/UX 設計」節、OpenSpec spec 必含可驗 UI/UX 驗收 scenario、QA 併驗 UX、review 加 uiux leg。
- **交互規則:UX 線不受裁剪**——`light + needs_uiux` 時 QA 保留(只驗 UI/UX 驗收 scenario,功能面靠 gate+review;`qa_skip` 會被引擎拒絕)、uiux leg 保留。
...
1. **Brainstorm**:先依「流程檔位與 UI/UX 線」判定兩軸(`flow_profile`/`needs_uiux`,uiux 準則見 `references/uiux-thread.md` §1)。**full**:產出設計文件——`superpowers=true` 用 `superpowers:brainstorming`,false 直接與使用者對話探索需求後撰寫;`needs_uiux=true` 時設計必含「UI/UX 設計」節(模板見 uiux-thread §2)。草稿寫 `.devloop/design-draft.md`(此時 change-id 未定;**不要**在 `docs/` 另存獨立設計文件)。`auto_approve=false` → ✋ 等使用者批准;true → 視為批准,直接進步驟 2。**light**:不產獨立草稿、跳過本步驟的 ✋——短設計節在步驟 2 直接寫入 proposal,兩軸判定值在✋批准提案時一併明示。
2. **Propose(OpenSpec)**:建立切小的 OpenSpec change(產生 change-id 與短命分支名),並把批准的設計文件移入 `openspec/changes/<id>/design.md` 當唯一正本(移入後刪草稿;light 檔位無獨立草稿——短設計節直接寫進 proposal.md);實作規劃直接寫 change 的 `tasks.md`,不另寫獨立 plan 文件。`needs_uiux=true` 時 spec 必含可驗的 UI/UX 驗收 scenario(寫法見 uiux-thread §3)。**兩軸判定值寫進 `.devloop/changes/<id>.json`**(`flow_profile`/`needs_uiux`)。過程 artifact(設計/計畫/delta spec)自此都活在 change 目錄,收尾 archive 時隨 change 整包進 `openspec/changes/archive/`,設計史集中一處。
3. **啟動引擎 + 驗證提案**:`devloop start --file .devloop/checkpoint.json --change-id <id> --branch <branch> --meta .devloop/changes/<id>.json --resume-exec "<續跑命令,如 claude -p '/dev-loop resume'>" --phase proposal_review`(`--meta` 凍結兩軸進 checkpoint);接著 `devloop validate-change --file .devloop/checkpoint.json` 以 strict 確認 change 結構合法。若 start 報 `checkpoint exists`(exit 2):既有 loop 尚未完結——先跑 `status` 判斷該 resume 還是升級給使用者,**不要**逕自 `--force` 覆蓋。
...
7. **QA Gate(QA subagent;可多情境平行)**:gate 全綠後 phase=qa,依兩軸三分岔(status 的 `next:` hint 已分好,照做即可):
   - **light 且非 uiux** → `devloop event --file .devloop/checkpoint.json --event qa_skip`(誠實跳過,引擎 guard 放行、history 記錄),進 review。
   - **needs_uiux=true**(含 light+uiux)→ QA subagent prompt 併入 uiux-thread §4 的 UX 檢查點;**light+uiux 時範圍縮為只驗 UI/UX 驗收 scenario**(功能面靠 gate+review)。
...
10. **收尾(finish 決策驅動)**:review 無 blocking 進入 merge phase 後,先問引擎決策:
   ...
   - **teardown(phase=teardown)**:整合完成後跑
     `devloop teardown --file .devloop/checkpoint.json --repo .`(mode 預設讀 checkpoint 的 `finish_mode`——`event --finish-mode` 已落地;`--mode <merge|pr>` 僅人工 override 用)
...
- 第二行是 `next: devloop event … --event qa_skip`(light 非 uiux 的 qa 裁剪)→ 直接執行即可;qa 的其他情境 hint 為 qa 命令骨架,依步驟 7 的分岔 dispatch QA subagent(uiux 時併 UX 檢查)。
- 第二行是 `next: devloop teardown …`(phase=teardown)→ 整合已完成、只差清殘留:照骨架執行即可(mode 預設讀 checkpoint 的 `finish_mode`,不必補 `--mode`;皆無時子命令會 exit 2 提示先跑 `event --finish-mode`),子命令會推進到 done。
