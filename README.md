...
| `gate --cmd "<cmd>" [...] [--timeout N]` | 依序跑命令;全綠→qa(exit 0),失敗→fix(exit 1),連續失敗超過 `--max-gate`→escalated(exit 3);失敗分支末行印 `phase=` |
...
續跑機制固定為 detached watcher:OS 程序(不依賴被卡住的 agent)反覆執行續跑命令,回 0 即停(loop 已推進),否則睡一個 heartbeat(預設 1800s、上限 3600s)再試。
